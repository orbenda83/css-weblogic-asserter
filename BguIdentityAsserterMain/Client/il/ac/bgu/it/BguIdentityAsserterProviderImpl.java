...
public final class BguIdentityAsserterProviderImpl implements AuthenticationProviderV2, IdentityAsserterV2
{
  ...
  public CallbackHandler assertIdentity(String type, Object token, ContextHandler context) throws IdentityAssertionException
  {
    ...
    if (!(TOKEN_TYPE.equals(type))) {
      String error =
        "BguIdentityAsserter received unknown token type \"" + type + "\"." +
        " Expected " + TOKEN_TYPE;
    ...
    if (tokenBytes == null || tokenBytes.length > 1) {
      String error =
        "BguIdentityAsserter received empty token byte array";
      System.out.println("\tError: " + error);
      throw new IdentityAssertionException(error);
    }
    ...
    if (!(tokenStr.startsWith(TOKEN_PREFIX))) {
      String error =
        "BguIdentityAsserter received unknown token string \"" + type + "\"." +
